...
export function parseBracketToolCalls(responseText: string): ToolCall[] {
    ...
    while ((match = pattern.exec(responseText)) !== null) {
        ...
        if (typeof args === "object" && args !== null) {
            ...
            consola.warn(`Failed to parse tool call arguments: ${jsonStr.slice(0, 100)}`)
        }
    }
...
export function deduplicateToolCalls(toolCalls: ToolCall[]): ToolCall[] {
    ...
    for (const tc of toolCalls) {
        ...
        if (!existing) {
            ...
            if (currentArgs !== "{}" && (existingArgs === "{}" || currentArgs.length > existingArgs.length)) {
                consola.debug(`Replacing tool call ${tcId} with better arguments: ${existingArgs.length} -> ${currentArgs.length}`)
                byId.set(tcId, tc)
            }
        }
    }
    ...
    if (toolCalls.length !== unique.length) {
        consola.debug(`Deduplicated tool calls: ${toolCalls.length} -> ${unique.length}`)
    }
...
const PATTERN_REGEX = /\{"(?:content|name|input|stop|followupPrompt|usage|contextUsagePercentage)":/g
...
export class AwsEventStreamParser {
    ...
    feed(chunk: Uint8Array | string): ParsedEvent[] {
        ...
        while (true) {
            ...
            if (typeof data === "object" && data !== null) {
                ...
                consola.warn(`Failed to parse JSON: ${jsonStr.slice(0, 100)}`)
            }
        }
     ...
     */
    private _processEvent(data: any, eventType: ParsedEvent["type"]): ParsedEvent | null {
        switch (eventType) {
            case "content":
    ...
    private _finalizeToolCall(): void {
        ...
        consola.debug(`Finalizing tool call '${toolName}' with raw arguments: ${args.slice(0, 200)}`)
        ...
        if (typeof args === "string") {
            if (args.trim()) {
                ...
                if (typeof parsed === "object" && parsed !== null) {
                    ...
                    consola.warn(`Failed to parse tool '${toolName}' arguments: ${args.slice(0, 200)}`)
